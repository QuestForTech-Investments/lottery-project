...
const useUserBlockedSessions = () => {
  ...
  const bloqueadosPorContrasena: BlockedItem[] = [
    {
      id: 1,
      usuario: 'juan.perez',
  ...
  const bloqueadosPorPin: BlockedItem[] = [
    {
      id: 1,
  ...
  const bloqueadosIP: BlockedItem[] = [
    {
      id: 1,
      ip: '192.168.1.200',
  ...
  // State
  const [activeTab, setActiveTab] = useState<'contrasena' | 'pin' | 'ip'>('contrasena');
   ...
   */
  const getCurrentTabData = useCallback((): BlockedItem[] => {
    switch (activeTab) {
      case 'contrasena':
   ...
   * Handle tab change
   */
  const handleTabChange = useCallback((_event: React.SyntheticEvent, newValue: 'contrasena' | 'pin' | 'ip') => {
