...
const useUserSessions = () => {
  ...
  const logsColisiones: SessionLog[] = [
    {
      id: 1,
      banca: 'Banca Central',
      usuario: 'juan.perez',
  ...
  const [activeTab, setActiveTab] = useState<'bancas' | 'colision'>('bancas');
   ...
   */
  const handleZoneChange = useCallback((event: any) => {
   ...
   * Handle tab change
   */
  const handleTabChange = useCallback((_event: React.SyntheticEvent, newValue: 'bancas' | 'colision') => {
