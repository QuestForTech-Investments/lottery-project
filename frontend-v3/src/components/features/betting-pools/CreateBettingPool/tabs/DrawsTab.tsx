/**
 * DrawsTab Component
 * Contains draw selection with chip-based UI
 * TypeScript version with full type safety
 * Performance optimized: draws loaded once in parent hook, eliminating duplicate API calls
 */
...
const DrawsTab: React.FC<DrawsTabProps> = ({ formData, handleChange, draws, loadingDraws }) => {
  // Sort draws by DRAW_ORDER
  ...
  const handleToggleAll = () => {
    const allDrawIds = sortedDraws.map((draw) => draw.drawId)
    const allSelected = sortedDraws.length > 0 && formData.selectedDraws?.length === sortedDraws.length
  ...
  const allSelected = sortedDraws.length > 0 && selectedCount === sortedDraws.length
  ...
  if (sortedDraws.length === 0) {
    return (
      <Box sx={{ p: 3 }}>
        <Alert severity="info">No hay sorteos disponibles</Alert>
      </Box>
    )
  }
          ...
          <Paper variant="outlined" sx={{ p: 3 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <Typography variant="body2" color="text.secondary">
                {selectedCount} de {sortedDraws.length} sorteo(s) seleccionado(s)
              </Typography>
            </Box>
          </Paper>
        </Grid>
...
const DrawMultiselect: React.FC<DrawMultiselectProps> = ({ draws, value = [], onChange, placeholder }) => {
  ...
  const handleToggleDraw = (drawId: number) => {
    let newValue: number[]
  ...
  const handleRemoveDraw = (drawId: number, e: React.MouseEvent) => {
    e.stopPropagation()
  ...
  const isAllSelected = filteredDraws.length > 0 && filteredDraws.every((draw) => value.includes(draw.drawId))
      ...
      {isOpen && (
        ...
        >
          ...
          <Box sx={{ p: 1, display: 'flex', gap: 1, borderBottom: 1, borderColor: 'divider' }}>
            <Button size="small" onClick={handleSelectAll} disabled={isAllSelected} sx={{ fontSize: '12px' }}>
              Seleccionar todas
            </Button>
            <Button size="small" onClick={handleClearAll} disabled={value.length === 0} sx={{ fontSize: '12px' }}>
              Limpiar
            </Button>
          </Box>
              ...
              {filteredDraws.map((draw) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isSelected && <Box sx={{ color: 'primary.main' }}>✓</Box>}
                  </Box>
                )
              })}
            </Box>
...
const arePropsEqual = (prevProps: DrawsTabProps, nextProps: DrawsTabProps): boolean => {
  ...
  // Check if draws changed
  if (prevProps.draws !== nextProps.draws) {
    return false
  }

  // Check if loadingDraws changed
