/**
 * Header Component
 * Top navigation bar with quick access buttons and user menu
 * Cloned from frontend-v2 with TypeScript typing (simplified version)
 */
...
const QuickAccessButton = React.memo<QuickAccessButtonProps>(({ Icon, label, path, index }) => {
...
function Header({ sidebarCollapsed, sidebarHovered, onToggleSidebar }: HeaderProps) {
  const navigate = useNavigate();
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
          {quickAccessButtons.map(({ Icon: IconComp, label, path }, idx) => (
            <QuickAccessButton key={idx} Icon={IconComp} label={label} path={path} index={idx} />
          ))}
        </Box>
