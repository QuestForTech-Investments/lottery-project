...
const sendToAnalytics = (metric) => {
  ...
  if (import.meta.env.DEV) {
    console.log(`%c📊 Web Vital: ${metric.name}`, 'color: #2196F3; font-weight: bold', metric);
  }
...
export const initWebVitals = async () => {
  try {
    ...
    if (webVitalsModule.onCLS) {
      webVitalsModule.onCLS(sendToAnalytics);
    }

    // Track First Input Delay (FID)
    // Good: < 100ms, Needs improvement: 100-300ms, Poor: > 300ms
    if (webVitalsModule.onFID) {
      webVitalsModule.onFID(sendToAnalytics);
    }

    // Track First Contentful Paint (FCP)
    // Good: < 1.8s, Needs improvement: 1.8-3s, Poor: > 3s
    if (webVitalsModule.onFCP) {
      webVitalsModule.onFCP(sendToAnalytics);
    }

    // Track Largest Contentful Paint (LCP)
    // Good: < 2.5s, Needs improvement: 2.5-4s, Poor: > 4s
    if (webVitalsModule.onLCP) {
      webVitalsModule.onLCP(sendToAnalytics);
    }

    // Track Time to First Byte (TTFB)
    // Good: < 800ms, Needs improvement: 800-1800ms, Poor: > 1800ms
    if (webVitalsModule.onTTFB) {
      webVitalsModule.onTTFB(sendToAnalytics);
    }

    // Track Interaction to Next Paint (INP) - New metric replacing FID
    // Good: < 200ms, Needs improvement: 200-500ms, Poor: > 500ms
    if (webVitalsModule.onINP) {
      webVitalsModule.onINP(sendToAnalytics);
    }
