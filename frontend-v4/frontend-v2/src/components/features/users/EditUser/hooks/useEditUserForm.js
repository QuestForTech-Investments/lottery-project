...
const useEditUserForm = (userId) => {
  ...
  const loadUserData = async () => {
    try {
      ...
      try {
        const permsResponse = await userService.getUserPermissions(userId);
        ...
        if (permsResponse.success && permsResponse.data) {
          ...
          const permIds = permissionsArray.map(p => p.PermissionId || p.permissionId);
   ...
   */
  const handleBranchChange = (bettingPoolId) => {
    setFormData(prev => ({
      ...prev,
   ...
   */
  const handleAssignBancaChange = (checked) => {
    setFormData(prev => ({
      ...prev,
      assignBanca: checked,
