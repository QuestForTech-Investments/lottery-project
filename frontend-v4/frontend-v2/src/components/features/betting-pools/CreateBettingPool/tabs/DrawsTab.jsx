 ...
 */
const DrawsTab = ({ formData, handleChange, draws: propDraws = [], loadingDraws: propLoadingDraws = false }) => {
      ...
      const loadLocalDraws = async () => {
        try {
          ...
          if (response.success && response.data) {
            const sortedDraws = response.data.sort((a, b) => {
            ...
            console.log(`✅ Loaded ${sortedDraws.length} draws locally (DrawsTab fallback)`);
          }
        } catch (err) {
 ...
 */
const DrawMultiselect = ({ draws, value = [], onChange, placeholder }) => {
  ...
  const handleRemoveDraw = (drawId, e) => {
    e.stopPropagation();
