...
const BettingPoolBalances = () => {
  ...
  const [selectedZones, setSelectedZones] = useState(MOCK_ZONES);
  ...
  const handleZonesChange = useCallback((event, newValue) => {
    setSelectedZones(newValue);
  }, []);

  const handleBalanceTypeChange = useCallback((event) => {
