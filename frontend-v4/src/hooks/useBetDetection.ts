export type BetGeneratorType = 'combinations' | 'sequence_pairs' | 'plus_100' | 'sequence'
...
export interface BetDetectionResult {
  ...
  displayName: string
  needsAmount: boolean
  generator?: BetGeneratorType
  sequenceStart?: string
  sequenceEnd?: string
}

/**
 * Hook para detectar automaticmente el bet type basándose en el formato del número ingresado
 * Replica la lógica de la aplicación Vue.js original
 */

export const useBetDetection = (input: string | null | undefined): BetDetectionResult => {
  ...
  // 1. Verificar generators especiales primero
  ...
  if (trimmedInput.endsWith('.')) {
    ...
    if (number.length === 4) {
      return {
        ...
        isValid: true,
        displayName: 'Pale Combinaciones',
        needsAmount: true,
        generator: 'combinations'
      };
    }
  ...
  // 2. Verificar modificadores position (F, B)
  ...
  // 4. Verificar modificadores basic (+, -)
  if (trimmedInput.endsWith('+')) {
    const number = trimmedInput.slice(0, -1);
    if (number.length === 3) {
      return {
  ...
  if (trimmedInput.endsWith('-')) {
    const number = trimmedInput.slice(0, -1);
    if (number.length === 4) {
      // Ambiguity: could be Play4 Straight or Panama
  ...
  // 5. Detection por longitud (without modifiers)
  ...
  if (length === 2) {
    // Could be Directo o Pick Two - by default Directo
    return {
      betType: 'DIRECTO',
      betTypeCode: 'DIRECTO',
      section: 'DIRECTO',
  ...
  if (length === 4) {
    // Could be Pale o Super Pale - by default Pale
    return {
      betType: 'PALE',
      betTypeCode: 'PALE',
      section: 'PALE & TRIPLETA',
