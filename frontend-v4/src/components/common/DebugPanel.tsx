...
import { getLogs, clearLogs, exportLogs, getLogsSummary, type LogEntry, type LogsSummary, type LogLevelValue } from '@/utils/logger'

/**
 * Debug Panel Component
 * Shows real-time logs for debugging
 */
type FilterLevel = LogLevelValue | 'ALL'
...
const DebugPanel = () => {
  ...
  const getLevelColor = (level: LogLevelValue) => {
    const colors: Record<LogLevelValue, string> = {
      INFO: '#2196F3',
      SUCCESS: '#4CAF50',
      WARNING: '#FF9800',
      ERROR: '#F44336',
    ...
    <div style={{
      ...
      <div style={{
        ...
        {['ALL', 'ERROR', 'WARNING', 'SUCCESS', 'INFO', 'DEBUG'].map((level) => (
          <button
            key={level}
            onClick={() => setFilter(level as FilterLevel)}
            style={{
              padding: '4px 10px',
              fontSize: '11px',
              border: 'none',
          ...
          >
            ...
            {summary && level !== 'ALL' && (
              <span style={{ marginLeft: '5px', opacity: 0.7 }}>
                ({summary.byLevel[level as LogLevelValue]})
              </span>
            )}
          </button>
        ))}
