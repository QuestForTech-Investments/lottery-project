...
const DebtCollectors = (): React.ReactElement => {
  ...
  const handleStatusChange = useCallback((status: keyof StatusFilters) => {
    setStatusFilters(prev => ({ ...prev, [status]: !prev[status] }));
  }, []);

  const handleCreateFormChange = useCallback((field: keyof CreateForm, value: string) => {
  ...
  const handleQuickFilterChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
    setQuickFilter(e.target.value);
  }, []);

  const handleTransactionDateChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
    setTransactionDate(e.target.value);
  }, []);

  const handleTipoChange = useCallback((e: SelectChangeEvent<string>) => {
    handleCreateFormChange('tipo', e.target.value);
  }, [handleCreateFormChange]);

  const handleBancoChange = useCallback((e: SelectChangeEvent<string>) => {
    handleCreateFormChange('banco', e.target.value);
  }, [handleCreateFormChange]);

  const handleTextFieldChange = useCallback((field: keyof CreateForm) => (e: ChangeEvent<HTMLInputElement>) => {
    handleCreateFormChange(field, e.target.value);
  }, [handleCreateFormChange]);
