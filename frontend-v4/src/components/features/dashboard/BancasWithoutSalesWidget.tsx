...
const BancasWithoutSalesWidget: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const rows = await getBancasWithoutSales(10);
              ...
              {data.map(row => (
                <TableRow key={row.bettingPoolId} hover>
                  ...
                  <TableCell align="center" sx={{ py: 0.5 }}>
                    <Chip
                      label={row.daysWithoutSales}
                      size="small"
                      sx={{ bgcolor: WARNING, color: 'white', fontWeight: 600, fontSize: 11, height: 22, minWidth: 36 }}
                    />
                  </TableCell>
                  <TableCell align="right" sx={{ fontSize: 12, color: balanceColor(row.balance), fontWeight: 600 }}>
...
export default BancasWithoutSalesWidget;
