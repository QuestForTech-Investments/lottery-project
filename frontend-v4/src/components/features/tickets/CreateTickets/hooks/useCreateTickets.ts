...
const useCreateTickets = () => {
  ...
  const [selectedSortition, setSelectedSortition] = useState<number>(61); // TEXAS MORNING by default

  // Settings
  const [discountEnabled, setDiscountEnabled] = useState<boolean>(true);
  const [multiLotteryEnabled, setMultiLotteryEnabled] = useState<boolean>(false);
  const [sendSmsEnabled, setSendSmsEnabled] = useState<boolean>(false);
  ...
  const [paleTripletaPlays, setPaleTripletaPlays] = useState<Play[]>([]);
  const [cash3Plays, setCash3Plays] = useState<Play[]>([]);
  const [play4Pick5Plays, setPlay4Pick5Plays] = useState<Play[]>([]);
   ...
   */
  const getTotals = useCallback((): Totals => {
    const allPlays: Play[] = [
      ...directoPlays,
      ...paleTripletaPlays,
      ...cash3Plays,
   ...
   */
  const getCurrentSortition = useCallback((): Sortition => {
   ...
   * Handle add play
   */
  const handleAddPlay = useCallback((): void => {
   ...
   * Handle delete play
   */
  const handleDeletePlay = useCallback((playId: number, type: PlayType): void => {
    switch (type) {
      case 'directo':
   ...
   */
  const handleDeleteAllPlays = useCallback((type: PlayType): void => {
    switch (type) {
      case 'directo':
        setDirectoPlays([]);
        break;
