 ...
 *
 * Allows users to enter/edit lottery results for a single draw.
 * Dynamically shows fields based on the draw category (USA vs standard).
 */
export const IndividualResultForm: FC<IndividualResultFormProps> = memo(({
  selectedDate,
  drawResults,
  form,
  saving,
  ...
  const handleDrawSelectChange = (event: SelectChangeEvent<number>) => {
    onDrawSelect(event.target.value as number);
  };

  const selectedDraw = drawResults.find((d) => d.drawId === form.selectedDrawId);
    ...
    <Box sx={{ p: 2, mb: 3, border: '1px solid #ddd', borderRadius: 1 }}>
      ...
      <Box sx={{ mt: 2 }}>
        <Button
          variant="contained"
          onClick={onPublish}
          disabled={!form.selectedDrawId || saving}
          sx={{
...
IndividualResultForm.displayName = 'IndividualResultForm';
