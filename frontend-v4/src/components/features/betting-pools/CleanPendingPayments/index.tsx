...
const CleanPendingPayments: React.FC = () => {
  ...
  const [cleanDate, setCleanDate] = useState<string>(new Date().toISOString().split('T')[0]);
  const [cleanSummary, setCleanSummary] = useState<CleanSummary>({ tickets: 0, amount: 0 });
  ...
  const handleReportSort = (property: string): void => {
  ...
  const handleCleanPayments = async (): Promise<void> => {
  ...
  const handleSearchReport = async (): Promise<void> => {
    if (!reportBancaId) {
  ...
  const reportTotals = useMemo((): { monto: number; premios: number } => {
    return filteredAndSortedReportData.reduce(
      (acc: { monto: number; premios: number }, item) => ({
