...
const ExcessesReport = (): React.ReactElement => {
  ...
  }, [selectedDraws, selectedBetTypes, availableDraws, availableBetTypes, mockReportData]);
  ...
  const handleDrawChange = useCallback((event: SelectChangeEvent<number[]>): void => {
    const value = event.target.value;
    setSelectedDraws(typeof value === 'string' ? value.split(',').map(Number) : value);
  }, []);

  const handleBetTypeChange = useCallback((event: SelectChangeEvent<number[]>): void => {
    const value = event.target.value;
