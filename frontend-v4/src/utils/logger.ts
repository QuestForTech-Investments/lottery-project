 ...
 * Log levels
 */
export const LogLevel: Record<LogLevelValue, LogLevelValue> = {
  INFO: 'INFO',
  SUCCESS: 'SUCCESS',
  WARNING: 'WARNING',
  ERROR: 'ERROR',
 ...
 */
const log = (level: LogLevelValue, category: string, message: string, data: unknown = null): void => {
  ...
  const styles: Record<LogLevelValue, string> = {
 ...
 * Success log
 */
export const success = (category: string, message: string, data?: unknown): void => {
 ...
 * Warning log
 */
export const warning = (category: string, message: string, data?: unknown): void => {
 ...
 */
export const startAutoExport = (intervalMinutes = 30): number => {
...
export interface LogsSummary {
  total: number
  byLevel: Record<LogLevelValue, number>
  lastLog?: LogEntry
}
