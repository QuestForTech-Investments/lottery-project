...
CREATE INDEX [IX_limit_consumption_near_limit] ON [dbo].[limit_consumption] ([is_near_limit], [is_at_limit]) WHERE [is_near_limit] = 1 OR [is_at_limit] = 1;
GO
...
CREATE INDEX [IX_hot_numbers_limits] ON [dbo].[hot_numbers] ([is_near_limit], [is_at_limit]) WHERE [is_near_limit] = 1 OR [is_at_limit] = 1;
...
BEGIN
    ...
    SELECT 
        ...
        COUNT(DISTINCT betting_pool_id) as unique_betting_pools
    FROM ticket_lines tl
    ...
    GROUP BY bet_number, lottery_id, draw_date;
END;
GO
...
GROUP BY u.user_id, u.username, u.full_name, u.email, r.role_name, r.role_id;
GO
