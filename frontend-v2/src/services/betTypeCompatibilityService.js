 ...
 * @param {string} drawName - The name of the draw
 * @returns {string} - The category name (DOMINICAN, USA, SUPER_PALE, PANAMA, ANGUILA)
 */
export function getDrawCategory(drawName) {
  ...
  for (const [category, draws] of Object.entries(DRAW_CATEGORIES)) {
    if (draws.some(d => d.toUpperCase() === normalizedName)) {
      return category;
    }
  }

  // Default: return ALL (General tab shows all bet types)
  return 'GENERAL';
}
 ...
 * @param {string} drawName - The name of the draw
 * @returns {string[]} - Array of allowed bet type codes
 */
export function getAllowedBetTypeCodes(drawName) {
 ...
 * @param {Array} betTypes - Array of bet type objects with betTypeCode property
 * @param {string} drawName - The name of the draw (or 'General')
 * @returns {Array} - Filtered bet types
 */
export function filterBetTypesForDraw(betTypes, drawName) {
 ...
 * @param {string} betTypeCode - The bet type code
 * @param {string} drawName - The draw name
 * @returns {boolean} - True if compatible
 */
export function isBetTypeCompatible(betTypeCode, drawName) {
