    ...
    const loadAllData = async () => {
      try {
        ...
        const [zonesResponse, bettingPoolsResponse, usersResponse] = await Promise.all([
          getAllZones({ page: 1, pageSize: 1000 }),
          getBettingPools({ page: 1, pageSize: 1000 }),
          getAllUsers({ page: 1, pageSize: 1000 })
        ]);

        // Process zones
        const zonesData = zonesResponse.success ? zonesResponse.data : zonesResponse;
        setZones(zonesData || []);

        // Process betting pools - map API response to expected format
        const bettingPoolsData = bettingPoolsResponse.success
          ? bettingPoolsResponse.data
          : bettingPoolsResponse.items || bettingPoolsResponse;

        const mappedBettingPools = (bettingPoolsData || []).map(pool => ({
        ...
        const usersData = usersResponse.success
          ? usersResponse.data
          : usersResponse.items || usersResponse;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const zonesToUpdate = Object.entries(zoneSelections).filter(
        ([_, data]) => data.bettingPools.length > 0 || data.users.length > 0
      );
